...
| `JS-Confuser-VM` (MBA #6) [/](./VM-8) Options: `{target: "browser", controlFlowFlattening: true, mba: true, minify: true, classObfuscation: true, handlerTable: true, randomizeOpcodes: true, shuffleOpcodes: true, encodeBytecode: true, concealConstants: true}` | Yes | Claude Opus 5 Extra High deobfuscated one-shot. MBA reversed by partial evaluation and probing VM handlers using the real per-instruction and per-function keys. Completed in 1 hour and 37 minutes, using 538,836 tokens. |
