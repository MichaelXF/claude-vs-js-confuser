...
| `JS-Confuser-VM` (MBA #1) with options `{target:"browser",controlFlowFlattening:true,mba:true,minify:true,specializedOpcodes:true,classObfuscation:true,handlerTable:true,randomizeOpcodes:true,shuffleOpcodes:true,encodeBytecode:true,concealConstants:true}` [/](./VM-3) | Yes | Claude Opus 5 Extra High deobfuscated one-shot. MBA reversed with a basic candidate operator table. |
| `JS-Confuser-VM` (MBA #2) with options `{target:"browser",controlFlowFlattening:true,mba:true,dispatcher:true,minify:true,specializedOpcodes:true,classObfuscation:true,handlerTable:true,randomizeOpcodes:true,shuffleOpcodes:true,encodeBytecode:true,concealConstants:true}` [/](./VM-4) | Yes | Claude Opus 5 Extra High deobfuscated one-shot. MBA reversed with numeric fitting. |
